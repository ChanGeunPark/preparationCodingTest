...
{
  const array = [1, 2, 3, 4, 5];
  const result = array.splice(0, 2); // splice는 기존 배열에서 완전 삭제해버린다. 반면 slice는 기존 배열은 그대로이다.
  console.log(result);
  console.log(array);
}
...
{
  const student = students.find((students) => students.score == 90);
  console.log(student);
}
...
{
  const result = students.map((students) => students.score).sort();
  console.log(result);
}
